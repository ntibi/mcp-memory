...
impl MemoryStore {
    ...
    pub async fn create(&self, input: CreateMemory, embedder: &dyn Embedder) -> Result<Memory> {
        ...
        self.conn
            .call(move |conn| {
                ...
                tx.execute(
                    ...
                    rusqlite::params![id_clone, content, created_at, updated_at],
                )?;
    ...
    pub async fn get(&self, id: &str) -> Result<Memory> {
        ...
        self.conn
            .call(move |conn| {
                ...
                let (id, content, created_at, updated_at) = memory
    ...
    pub async fn list(&self, filter: ListFilter) -> Result<Vec<Memory>> {
        ...
        self.conn
            .call(move |conn| {
                ...
                for id in ids {
                    let mut stmt = conn.prepare(
                        "SELECT content, created_at, updated_at FROM memories WHERE id = ?1",
                    )?;
                    let (content, created_at_str, updated_at_str) = stmt.query_row(
                        rusqlite::params![id],
                        |row| {
                            Ok((
                                row.get::<_, String>(0)?,
        ...
        self.conn
            .call(move |conn| {
                ...
                let changed = tx.execute(
                    ...
                    rusqlite::params![content, updated_at_str, id],
                )?;
    ...
    pub async fn vote(&self, memory_id: &str, vote: &str) -> Result<Vote> {
    ...
    pub async fn set_tags(&self, memory_id: &str, tags: Vec<String>) -> Result<()> {
        let memory_id = memory_id.to_string();
        self.conn
            .call(move |conn| {
                let tx = conn.transaction()?;
    ...
    pub async fn search_by_tag(&self, tag: &str, limit: usize) -> Result<Vec<Memory>> {
        ...
        self.conn
            .call(move |conn| {
                ...
                for (id, content, created_at_str, updated_at_str) in raw {
                    let tags = load_tags(conn, &id)?;
                    let created_at: DateTime<Utc> = created_at_str
                        .parse()
                        .map_err(|e: chrono::ParseError| {
        ...
        let memory_data: Vec<(Memory, u64, u64)> = self
            ...
            .call(move |conn| {
                ...
                for id in &ids_for_db {
                    ...
                    let (content, created_at_str, updated_at_str) = match row {
                        Ok(r) => r,
                        Err(_) => continue,
                    };
...
mod tests {
    ...
    async fn should_update_memory_content() {
        ...
        let updated = store
            .update(&created.id, "updated content", &embedder)
            .await
            .unwrap();
    ...
    async fn should_cast_vote_on_memory() {
        ...
        let vote = store.vote(&created.id, "helpful").await.unwrap();
    ...
    async fn should_reject_invalid_vote() {
        ...
        let result = store.vote(&created.id, "neutral").await;
    ...
    async fn should_set_tags_replacing_existing() {
        ...
        let m = store
            ...
            .unwrap();
        store
            .set_tags(&m.id, vec!["new1".into(), "new2".into()])
            .await
            .unwrap();
        let fetched = store.get(&m.id).await.unwrap();
    ...
    async fn should_get_vote_counts() {
        ...
        let m = store
            ...
            .await
            .unwrap();
        store.vote(&m.id, "helpful").await.unwrap();
        store.vote(&m.id, "helpful").await.unwrap();
        store.vote(&m.id, "harmful").await.unwrap();
    ...
    async fn should_recall_similar_memories() {
        ...
        let results = store.recall("rust code", 3, &embedder, &scorer).await.unwrap();
