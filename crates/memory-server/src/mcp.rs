    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions(concat!(
                ...
                "RECALL TRIGGERS — you MUST call `recall_memory` or `search_by_tags` in ALL of these situations: ",
                "(1) before implementing any feature or fix, recall the module/technology you're about to touch; ",
                "(2) before debugging, recall the error message, module, or symptom; ",
                "(3) before choosing a library, tool, or pattern — past sessions may have evaluated it; ",
                "(4) when reading unfamiliar code, recall the file path or module name; ",
                "(5) when the user references something from a past session, recall it — don't guess; ",
                "(6) when context-switching to a different part of the codebase; ",
                "(7) before answering architecture or design questions — prior decisions may exist; ",
                "(8) when encountering an error or unexpected behavior, recall the error text; ",
                "(9) before suggesting a workaround — a known fix may already exist; ",
                "(10) when unsure about a user preference or convention — recall rather than ask; ",
                "(11) before answering any user question — you may have context from prior sessions that improves your answer; ",
                "(12) before asking the user a clarifying question — the answer may already exist in memory from a prior session. ",
                "Default bias: a wasted recall costs nothing, a missed recall means repeating past mistakes. When in doubt, ALWAYS recall. ",
